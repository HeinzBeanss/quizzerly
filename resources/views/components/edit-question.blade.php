    ...
    <div class="buttoncontainer flex gap-4 mt-4">
        @if ($index === 0)
            <button type="button" onclick="addAnswer({{ $index }})"
                class="px-4 py-2.5 bg-gradient-to-br from-lighter/90 via-surface to-background/70 rounded-md min-w-40 text-sm font-light tracking-wide hover:font-normal hover:from-lighter hover:to-background/70 hover:shadow">Add
                Answer</button>
        @else
            <button type="button" onclick="addAnswer({{ $index }})"
                class="px-4 py-2.5 bg-gradient-to-br from-lighter/90 via-surface to-background/70 rounded-md min-w-40 text-sm font-light tracking-wide hover:font-normal hover:from-lighter hover:to-background/70 hover:shadow">Add
