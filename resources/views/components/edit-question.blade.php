    ...
    <div id="questionContainerBottom{{ $index }}" class="flex flex-col gap-4">
        @foreach ($question->answers as $answerIndex => $answer)
            @if ($answerIndex === 0 || $answerIndex === 1)
                <input type="text" name="question[{{ $index }}][{{ $answerIndex }}]" required
                    class="max-h-24 pt-3 pb-3 rounded-xl" value="{{ $answer->name }}">
            @else
                <div id="answer[{{ $index }}][{{ $answerIndex }}]" class="flex justify-between gap-4 w-full">
                    <input type="text" name="question[{{ $index }}][{{ $answerIndex }}]" required
                    ...
                    <button
                        ...
                        type="button"
                        onclick="deleteAnswer({{ $answerIndex }}, {{ $index }})">Delete</button>
                </div>
            @endif
        @endforeach
    </div>
