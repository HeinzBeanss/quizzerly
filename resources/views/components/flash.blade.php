@if (session()->has('success'))
    ...
    setTimeout(() => show = false, 5000)" x-show="show"
        ...
        class="fixed bg-gradient-to-br from-surface to-background text-words text-lg py-4 px-8 rounded-xl top-4 left-1/2 transform -translate-x-1/2 text-sm border border-white z-50">
        <p>{{ session('success') }}</p>
    </div>
@endif
